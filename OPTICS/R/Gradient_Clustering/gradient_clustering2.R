...
gradient_clustering <- function(co, reachdist, minPts, t) {
 ## has to be removed when function ready
  co <- list()
  startPts <- rstack()
  currentCluster <- list()
  setOfClusters <- list(currentCluster)
 i =0
  co<- c(i)
  o <- co[i]
  print(co)
  startPts <- insert_top(startPts, i)

  ## pushing the index of the point instead of the point itself
  i=i+1
  while(i <length(co)){
    co <-c(i)
    o <- co[i]
